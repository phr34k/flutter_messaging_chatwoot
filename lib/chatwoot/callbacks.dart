...
class ChatwootHttpClient {
  Future<ChatwootContact> createContact() {
    return Future<ChatwootContact>.value(ChatwootContact.fromJson({
      "id": 0,
      "source_id": "string",
      "name": "string",
      "email": "string",
...
class ChatwootCallbacks {
  ...
  void Function(int conversationId)? onConversationStartedTyping;
  ...
  ///is received after connecting to the chatwoot websocket. See [ChatwootRepository.listenForEvents]
  void Function(int conversationId)? onConversationStoppedTyping;
  ...
  ///Triggered when an agent resolves the current conversation
  void Function(int conversationId)? onConversationResolved;
